...
if session:ready() then
  ...
  channel["accountcode"] = 2--get_Variable("accountcode")
  ...
  channel["caller_id_number"] = "380931707180" --get_Variable("caller_id_number")
...
-----------------------------------------------
  ...
  freeswitch.consoleLog("info", query_cost_sql);
  log("SQL: ", query_cost_sql)
  assert(dbh:release())
  lcrok = lcrok - 1
...
--    if (session:ready() == true) then
      ...
      --called_number = string.gsub(called_number, "38", "", 1)
      ...
      if mydialbridge == "" then
        mydialbridge = "{ignore_early_media=false}[" .. myvarbridge .. "]sofia/gateway/" .. lcr_gwname[i] .. "/" .. called_number
